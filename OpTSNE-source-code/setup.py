...
annoy_path = "OpTSNE/dependencies/annoy/"
annoy = Extension(
    "OpTSNE.dependencies.annoy.annoylib",
...
# Other extensions
extensions = [
    Extension("OpTSNE.quad_tree", ["OpTSNE/quad_tree.pyx"], language="c++"),
    Extension("OpTSNE._tsne", ["OpTSNE/_tsne.pyx"], language="c++"),
    Extension("OpTSNE.kl_divergence", ["OpTSNE/kl_divergence.pyx"], language="c++"),
    annoy,
]
...
if has_c_library("fftw3"):
    ...
    extension_ = Extension(
        "OpTSNE._matrix_mul.matrix_mul",
        ["OpTSNE/_matrix_mul/matrix_mul_fftw3.pyx"],
        libraries=["fftw3"],
        language="c++",
    )
    ...
    extension_ = Extension(
        "OpTSNE._matrix_mul.matrix_mul",
        ["OpTSNE/_matrix_mul/matrix_mul_numpy.pyx"],
        language="c++",
    )
    extensions.append(extension_)
...
exec(open(os.path.join("OpTSNE", "version.py")).read())

setup(
    name="OpTSNE",
    description="implementations of t-SNE with the ability to optimise for the gradient of DoF parameter ",
    ...
    packages=setuptools.find_packages(include=["OpTSNE", "OpTSNE.*"]),
    python_requires=">=3.9",
    install_requires=[
        "numpy>=1.16.6",
        "scikit-learn>=0.20",
